...
---
**Just a reminder: we’re communicating with internal services across different companies and regions via VNet peering.**

## Manual Steps:

1. Create 3 Resource Groups (RGs):
   - france RG in `francecentral`
   - india RG in `southindia`
   - us RG in `westus`
  
2. Create 3 VNETs in each region with subnets and NSG:
   - for france : france-vnet:
     - public-subnet(`10.0.0.0/24`) & france-public NSG 
     - private-subnet(`10.0.1.0/24`) & france-private NSG
   - for india : 
     - india-vnet -> private-subnet(`172.16.0.0/24`) & india-private NSG
   - for us: 
     - us-vnet -> private-subnet(`192.168.0.0/24`) & us-private NSG
  
3. Go to Network Security Group(NSG) : Edit `france-public`NSG -> 
   - Go to Inbound Security rules
   - Add
   - Select `My IP Address` in Source + in Destination port range, Set Destination Port Ranges to 80,3389
...
5. Create a linux server in each private subnet and no need to create new NSGs or public IPs.
...
1. Customize the default page slightly to differentiate between Apache servers.
   
2. Virtual network:
   - select `france vnet`> peerings > ADD 
      -(Remote VNET Summary) Peering link name : `india-to-france`
      - Virtual network: select `India Vnet`
      - Select first two options: `Allow virtual network access from India to France` + `Allow forwarded traffic from India to be received by France`
...
3. Do the same to peer `france and us` + `india and us`
  
4.  Go to Private DNS Zones
    - Create in france RG: `paul.lab` <- choose any domain name you want
    - Add all our Vnet to the Vnet Link before the creation
    - I personally don't select the `Enable auto registration`
    - Create
    - Go to that domaine:
      - Recordsets: 
      - Add : `windows-france` in Name + `10.0.0.4` in IP Address to add your windows machine to the domaine
      - Add similar records for other servers using their respective private IPs
  
5.  Take the `RDP` to the Windows machine with `public ip`
   
6.  Install `IIS` on the Windows server manually
   
7.  Test the web servers using Edge: `http://[DNS Record]/[DNS Zone name]`
   
8.  Delete all Three RGS.

---

**Personal note: The manual setup took me 1 hour 20 minutes.**
****👉 TRY TERRAFORM 😊 (10 minutes)**